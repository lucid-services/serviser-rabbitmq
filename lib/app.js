...
App.prototype.$getBrokerURI = function() {
    /* FORMAT
     * amqp://username:password@host:port/vhost?optional=param
     */
    let amqp       = this.config.getOrFail('amqp')
    ,   options    = ['heartbeat', 'channelMax', 'frameMax']
    ...
    if (host) {
        ...
        //append optional tunning parameters as query string
        let query = options.reduce(function(out, key) {
            if (typeof amqp[key] !== 'undefined') {
                out += `${key}=${amqp[key]}`;
            }
            return out;
        }, '');
        ...
        app.connection.once('ready', function(channel) {
            app.routers.forEach(function(router) {
                router.routes.forEach(function(route) {
                    ...
                    const socket = app.connection.socket(
                        route.options.type.toUpperCase(),
                        route.options.amqp
                    );
                    route.$socket = socket;
...
/**
 * @return {Object} serviser-rabbit connection Context object
 */
...
App.prototype.close = Promise.method(function() {
    ...
    Object.keys(this.routes || {}).forEach(function(uid) {
        this.routes[uid].$socket.close();
    }, this);

    return Promise.fromCallback(function(cb) {
        app.connection.close(cb);
    });
});
